...
spritesheet_animation_speed = 0.2
spritesheet_animation_timer = 0
...
while running:
    ...
    circle_sprites.update(dt) # обновление для круглишков

    # --- Анимация спрайт листа ---
    spritesheet_animation_timer += spritesheet_animation_speed
    if spritesheet_animation_timer >= 1:
        spritesheet_animation_timer = 0
        spritesheet_current_frame = (spritesheet_current_frame + 1) % len(spritesheet_frames)
    # -----------

    screen.fill((0, 0, 0))
    screen.blit(square_frames[current_frame], (100, 100)) # отрисовка квадрата
    circle_sprites.draw(screen) # отрисовка круглишков
    screen.blit(spritesheet_frames[spritesheet_current_frame], (50, 400)) # отрисовка спрайт листа
